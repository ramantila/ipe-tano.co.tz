...
<head>
    ...
    <title>IPE TANO | {{ $company->business_name }}</title>
                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            <form action="" id="reviewForm">
                                <div class="box_general write_review">
                                    <h4>{{ __('messages.write_review') }} {{ __('messages.for') }} {{ $company->business_name }}</h4>
                                    <input class="form-control" id="company_id" name="company_id" type="text"
                                    ...
                                    @else
                                        <div class="rating_submit">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-md-6">
                                                            <label class="d-block "
                                                                style="color: transparent;">Overall
                                                                rating</label>
                                                            <h5 class="d-block" id="displayhere"></h5>
                                                        </div>
                                                    </div>
                                            ...
                                            <input class="form-control" name="company_id" type="text"
                                                value="{{ Session::get('company_id') }}" hidden>
                                            <div class="form-group">
                                                <label>{{ __('messages.your_review') }}</label>
                                                <textarea class="form-control" style="height: 180px;" name="review_input"
                                                    placeholder="{{ __('messages.your_review') }}" required>{{ Session::get('review_input') }}</textarea>
                                            </div>
                                        </div>
                                    @endif
                        ...
                        <div class="col-lg-4">
                            <div class="latest_review">
                                <h4>{{ __('messages.recent_reviews_for') }} {{ $company->business_name }}</h4>
...
<script>
    const categoryTranslations = @json(__('messages.categories_list'));
</script>
    <script>
        function fetchRecentReviews() {
            $.ajax({
                url: '/consumer/review-list/{{ $company->id }}', // The URL to the Laravel route
                type: 'GET',
                dataType: 'json',
        ...
        function updateReviewsCard(reviews) {
            ...
            for (var i = 0; i < reviews.length; i++) {
                ...
                var reviewHTML =
                    ...
                    '<figure><img src="' + review.avatar_url + '" alt=""></figure>' +
                    '<span class="rating ">' + getStarIcons(review.rating) + '<em>' +
                    review.rating + '/5.00</em></span>' +
                    '<small>' +categoryTranslations[review.business.category.category_name.toLowerCase().replace(/ /g, "_")]  + '</small>' +
                    '</div>' +
                    '<h3><strong>' + review.user.first_name + ' ' + review.user.last_name + '</strong></h3>' +
                    '<h4>"' + review.review_title + '"</h4>' +
                    '<p>' + review.review + '</p>' +
                    '<ul class="clearfix">' +
                    '<li><small>{{ __('messages.haliliwa') }}' + formatDateToDMY(review.created_at) + '</small></li>' +
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.message == "success") {
                            ...
                            $("#alertMessage").html(
                                '<div class="alert alert-warning" role="alert">You have already reviewed this business.!</div>'
                            );
                        }
                        else {
                            // console.log(response)
